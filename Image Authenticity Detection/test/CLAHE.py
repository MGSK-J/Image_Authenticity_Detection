...
def prepare_image_clahe(image_path, target_size=(64, 64)):
    return np.array(convert_to_clahe(image_path).resize(target_size)) / 255.0
...
for img_name in os.listdir(au_path)[:2000]:
    ...
    if img_path.lower().endswith(('.jpg', '.png', '.tif')):
        X.append(prepare_image_clahe(img_path))
        Y.append(1)
...
def display_samples(image_paths, title):
    ...
    for i, img_path in enumerate(image_paths[:5]):
        ...
        plt.axis('off')
        plt.subplot(2, 5, i+6)
        clahe_img = convert_to_clahe(img_path)
...
display_samples(au_images, "Authentic Images - Original vs CLAHE")
tp_images = [os.path.join(tp_path, f) for f in os.listdir(tp_path) if f.lower().endswith(('.jpg', '.png', '.tif'))][:5]
display_samples(tp_images, "Tampered Images - Original vs CLAHE")
...
model.save('clahe_model.h5')
print("Model saved as 'clahe_model.h5'")
...
def predict_image(model, image_path):
    image = prepare_image_clahe(image_path)
