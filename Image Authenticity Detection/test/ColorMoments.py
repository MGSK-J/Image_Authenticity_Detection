...
def convert_to_color_moments(path):
    ...
    for c in range(3):
        ...
        std = np.std(channel)
        skw = skew(channel)
        moments.extend([mean, std, skw])
    feat_img = np.ones((64, 64, 3), dtype=np.float32)
    for i in range(3):
        feat_img[..., i] *= moments[i*3] / 255.0
    return Image.fromarray(np.clip(feat_img*255, 0, 255).astype(np.uint8))

def prepare_image_colormoments(image_path, target_size=(64, 64)):
    return np.array(convert_to_color_moments(image_path).resize(target_size)) / 255.0
...
for img_name in os.listdir(au_path)[:2000]:
    ...
    if img_path.lower().endswith(('.jpg', '.png', '.tif')):
        X.append(prepare_image_colormoments(img_path))
        Y.append(1)
...
def display_samples(image_paths, title):
    ...
    for i, img_path in enumerate(image_paths[:5]):
        ...
        plt.axis('off')
        plt.subplot(2, 5, i+6)
        color_moments_img = convert_to_color_moments(img_path)
        plt.imshow(color_moments_img)
        plt.title(f"Color Moments {i+1}")
        plt.axis('off')
    plt.tight_layout()
    plt.show()

au_images = [os.path.join(au_path, f) for f in os.listdir(au_path) if f.lower().endswith(('.jpg', '.png', '.tif'))][:5]
display_samples(au_images, "Authentic Images - Original vs Color Moments")
tp_images = [os.path.join(tp_path, f) for f in os.listdir(tp_path) if f.lower().endswith(('.jpg', '.png', '.tif'))][:5]
display_samples(tp_images, "Tampered Images - Original vs Color Moments")
...
model.save('color_moments_model.h5')
print("Model saved as 'color_moments_model.h5'")
...
def predict_image(model, image_path):
    image = prepare_image_colormoments(image_path)
