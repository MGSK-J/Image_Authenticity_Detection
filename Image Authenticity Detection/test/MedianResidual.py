...
def convert_to_median_residual(path):
...
def prepare_image_median(image_path, target_size=(64, 64)):
    return np.array(convert_to_median_residual(image_path).resize(target_size)) / 255.0
...
for img_name in os.listdir(au_path)[:2000]:  # Limit to 2000 for faster processing
    ...
    if img_path.lower().endswith(('.jpg', '.png', '.tif')):
        X.append(prepare_image_median(img_path))
...
def display_samples(image_paths, title):
    ...
    for i, img_path in enumerate(image_paths[:5]):
        ...
        # Median Residual Processed Image
        plt.subplot(2, 5, i+6)
        med_img = convert_to_median_residual(img_path)
        plt.imshow(med_img)
        plt.title(f"Median Residual {i+1}")
        plt.axis('off')
...
display_samples(au_images, "Authentic Images - Original vs Median Residual")

# Tampered images
tp_images = [os.path.join(tp_path, f) for f in os.listdir(tp_path) 
             if f.lower().endswith(('.jpg', '.png', '.tif'))][:5]
display_samples(tp_images, "Tampered Images - Original vs Median Residual")
...
# Save model
model.save('median_residual_model.h5')
print("Model saved as 'median_residual_model.h5'")
...
def predict_image(model, image_path):
    image = prepare_image_median(image_path)
